...
var tapHolded = false;
var tapHoldedlong = false;
var tapMoved = false; 

var isnewrequest=false;
var foldermake_inprogress=false;
var disable_desktop_refresh=false;

		var mouseX_new_icon=0;
		var mouseY_new_icon=0;
...
//******************************************************************************
function save_datafile(DATAURL,MEMORY_DATA){
   ///saving_desktopini=true;
   //console.log("url:"+DATAURL);
   $.ajaxSetup({
      cache: false,
      async: false //wait for ajax end
   });
   $.ajax({
      type: 'POST',    
      url:'./app/savedata.php',
      data:{'dataurl':DATAURL,'memories':MEMORY_DATA},
      beforeSend: function( xhr ) {
         xhr.overrideMimeType( "text/plain; charset=x-user-defined" );
      },
      success: function(msg){
         //console.log("*** save_datafile success");
		 ///saving_desktopini=false;
      },
      error: function(msg){
         //console.log("*** save_datafile write error");
		 ///saving_desktopini=false;
      }
   });
}
//******************************************************************************
//******************************************************************************
function load_datafile(DATAURL) {
   var load_tmpdata="";
   $.ajaxSetup({
      cache: false,
      async: false //wait for ajax end
   ...
   $.ajax({
      ...
      success: function(dataout){
         load_tmpdata=dataout;
         //console.log("*** load_datafile success");
      },   
      error: function(msg){
         //console.log("*** load_datafile error");
         load_tmpdata=false;
      }
   });
   //} catch(err) {
...
function checkurl(DATAURL) {
   ...
   return check_tmpdata;
}
//******************************************************************************
//******************************************************************************
//NCX: Added touch devices support.
//******************************************************************************
   ...
   switch(event.type) {
      case "touchstart": 
        if (!tapedTwice) {
         ...
         setTimeout( function() { if (tapHolded==true) tapHoldedlong=true;  }, 1000 );
         tapHolded=true;
         tapMoved=false;  
         break;
      case "touchmove":  
         ...
         break;        
      case "touchend":   
         if (tapMoved || tapHoldedlong) {
            type="mouseup"; 
         } else {
            //NCX HACK! click only for non A href elements (need for task bar, etc.)
...
define("app/src/desktop/main", ["lib/jquery-lib", "lib/util", "lib/contextMenu/jquery-contextMenu", "lib/artDialog/jquery-artDialog", "../../common/taskTap", "../../common/core", "../../common/tpl/upload.html", "../../common/tpl/formMake.html", "../../common/core.tools", "../../common/core.upload", "../../common/core.api", "../../common/core.playSound", "../../common/core.formMake", "../../common/rightMenuExtence", "../../app/appBase", "../../app/editor", "../../app/openWith", "../../app/html", "../../common/tpl/copyright.html", "../../common/tpl/themeDIY.html", "../../common/rightMenu", "../explorer/ui", "../explorer/fileContent", "../../path/tpl/file/list.html", "../../path/path", "../../path/pathOperate", "../../path/tpl/share.html", "../../path/tpl/fileinfo/fileInfo.html", "../../path/tpl/fileinfo/pathInfo.html", "../../path/tpl/fileinfo/pathInfoMore.html", "../../path/tpl/appEdit.html", "../../path/clipboard", "../../path/search", "../../path/tpl/search.html", "../../path/tpl/searchList.html", "../../path/tpl/file/create.html", "../explorer/fileLight", "../explorer/fileSelect", "../explorer/fileListResize", "../explorer/options"], function(a, b, c) {
    Config = {
        ...
        navbar: "navbar",
        AnimateTime: 200
    }, a("lib/jquery-lib"), a("lib/util"), a("lib/contextMenu/jquery-contextMenu"), a("lib/artDialog/jquery-artDialog"), TaskTap = a("../../common/taskTap"), core = a("../../common/core"), rightMenu = a("../../common/rightMenu"), ui = a("../explorer/ui"), ui.path = a("../../path/path"), ui.fileLight = a("../explorer/fileLight"), ui.fileSelect = a("../explorer/fileSelect"), ui.fileListResize = a("../explorer/fileListResize"), ui.options = a("../explorer/options");
    $(document).ready(function() {
//******************************************************************************   
//NCX: init touchHandlers 
//******************************************************************************   
...
//****************************************************************************** 
        rightMenu.initDesktop(), G.userConfig.listType = "icon", core.init(), ui.init(), ui.fileLight.init(), ui.fileSelect.init(), TaskTap.init(), ui.fileListResize.initFileSize(), ui.options.init(), $(".init-loading").fadeOut(450).addClass("pop_fadeout"), $(".file-continer").removeClass("hidden"), a.async("lib/webuploader/webuploader-min", function() {
            core.uploadInit()
        }), $(".bodymain").click(function(a) {
            "block" == $("#menuwin").css("display") && $("#menuwin").css("display", "none"), $("body").focus()
            ...
            core.copyright()
        }), $(".tab-hide-all").click(function() {
            if (0 != $.dialog.list.length) {
                $(this).toggleClass("this");
                var a = !$(this).hasClass("this");
                $.each($.dialog.list, function(b, c) {
                    c.display(a)
                })
            }
        });
//******************************************************************************                		
        var b = '<div id="random-wallpaper">			<img class="flower animated-1000" src="' + G.staticPath + 'images/common/desktop/fengche.png" title="' + LNG.setting_wall + '">			<div class="body-line" ></div>		</div>';
        $("body").append(b);
        var c = $("#random-wallpaper .flower");
        c.bind("click", function() {
            c.addClass("moveCircle");
            var a = function(a) {
            ...
            core.api.randomImage(function(b) {
                ui.setWall(b, function() {
                    setTimeout(function() {
                        c.removeClass("moveCircle")
                    }, 100)
                }), $.get(G.appHost + "setting/set&k=wall&v=" + urlEncode(b)), a(b)
            })
        })
    })
});;;;;;
...
//NCX call refresh desktop after upload********************************************************************************
                    isnewrequest=true;
					//ui.resetDesktopIcon(); //uprac iconky
                },
                init: function() {
                    var r = function(n, i) {
                            var t = new $[Yt];
...
define(_kod_0x200f('0x0', 'Wk0G'), [], function(_0x138506, _0x5b11da) {
    ...
    $[_kod_0x200f('0x3d', 'tM4G')] || ($[_kod_0x200f('0x3e', 'JewW')] = {}), $[_kod_0x200f('0x3f', '!BNj')][_kod_0x200f('0x40', 'WpY(')] = function(_0x193d7d, _0x910500, _0x1b7504) {
        ...
        if (_kod_0x200f('0x63', 'HLSS') === _kod_0x200f('0x64', '%lV(')) {
            ...
            for (var _0x486bef in _0x101a8e)
                if (_0x101a8e[_0x486bef][_kod_0x200f('0x69', '*O7U')] == _0xcc7eea || _0x101a8e[_0x486bef][_kod_0x200f('0x6a', 'j90Z')] == _0x49a69a[_kod_0x200f('0x6b', 'mfiB')]('.', _0xcc7eea) || _0x49a69a['YpeLF'](_0x101a8e[_0x486bef][_kod_0x200f('0x6c', 'SNFI')], '#' + _0xcc7eea)) {
        ...
        if (_0x49a69a[_kod_0x200f('0xd6', 'JewW')]('pxplF', _kod_0x200f('0xd7', 'j90Z'))) {
            ...
            for (var _0x4353ec in _0x13aa0e)
                if (_0x13aa0e[_0x4353ec][_kod_0x200f('0xda', 'g5gk')] == _0x4c95e6) {
    ...
    var c = a("./fileContent"),
        ...
        t = function() {
            if (core.isApp("desktop")) {
                ...
                var icontops = new Array();
                var iconlefts = new Array();				
				var tmp=0;
				//icontops[0]=20;
				//iconlefts[0]=20;
				fileelements[0]=""
                ...
                var iniurl="";
                if (G.myDesktop=="/desktop/") {
                   iniurl=("../"+"data/User/"+$("#topbar-user").text().trim()+"/home"+G.myDesktop+'desktop.ini');
                } else {
                   iniurl=G.myDesktop+'desktop.ini';
                   iniurl=core.path2url(iniurl);
                }

                /*if ($(".box-size-smallx").hasClass("selected")) iconsize=1; 
                else if ($(".box-size-small").hasClass("selected")) iconsize=2; 
                else if ($(".box-size-default").hasClass("selected")) iconsize=3; 
				...
				icontops[0]=20;
				iconlefts[0]=20;
				//if (iconsize==0) $(".file-continer").hide();
							
//******************************************************************************  				
				var a = 20,
                    b = 20,
                    c = parseInt($(".file").css("height")), //Y zakladna pozicia iconiek
                    d = c - 30,
                    e = 10,
                    f = 15,
                ...
                m > 0 && (e += m), $(".file-continer .file").css("position", "absolute"), $(".file-continer .file").each(function(g) {
                    i = g % h, j = Math.floor(g / h), k = b + (d + f) * j, l = a + (c + e) * i; 

				if (/*checkurl(iniurl) &&*/ desktopinidata=="") desktopinidata = load_datafile(iniurl); //NCX MAIN LOAD FIX
				if (desktopinidata!="") {
                    filelines = desktopinidata.split("\n");                                             
                    var fileelements="";
				    var DestopFileEQ=-1;
						    if (iconsize>0) {   
                            ///var filenotfound="";
							var filematch=false;
   							$.each( filelines, function( mc, mcm ) {
							    if (mcm!="") {
							        fileelements=mcm.split("|");
									if ($(".file-continer .file").eq(g).find(".filename .title").text().trim()== fileelements[0].toString()) {
										//match file
										filematch=true;
										DestopFileEQ=g;
										return(false);
									} else if ($(".file-continer .file").eq(g).find(".filename .title").text().trim() == newfilename)  {
										//new File or Folder
										filematch=true;
										DestopFileEQ=g;
								        fileelements[1]=parseInt(mouseY_new_icon,10);
                                        fileelements[2]=parseInt(mouseX_new_icon,10);									   
										newfilename="";
										return(false);
									} 
								}	
                            ...
                            if (!filematch) {
							    //Renamed File on Desktop
								DestopFileEQ++;
								fileelements[1]=Math.round(parseInt(newfilenameY,10));
								fileelements[2]=Math.round(parseInt(newfilenameX,10));
							}			
						   
                            fileelements[1]=Math.round(parseInt(fileelements[1],10)); //top
                            fileelements[2]=Math.round(parseInt(fileelements[2],10)); //left
							


                            //fit to grid disordered icons on desktop************************************************************************
                            var b=0;
                            if (iconsize==1) {
                                if (((fileelements[1]-20)%84)>40) fileelements[1]=fileelements[1]-((fileelements[1]-20)%84)+84;  
                                else fileelements[1]=fileelements[1]-((fileelements[1]-20)%84);  
                                if (((fileelements[2]-20)%55)>30) fileelements[2]=fileelements[2]-((fileelements[2]-20)%55)+55;  
                                else fileelements[2]=fileelements[2]-((fileelements[2]-20)%55);    
                                fileelements[1]=fileelements[1]+b*84;                
                            } else if (iconsize==2) {
                                if (((fileelements[1]-20)%104)>70) fileelements[1]=fileelements[1]-((fileelements[1]-20)%104)+104;  
                                else fileelements[1]=fileelements[1]-((fileelements[1]-20)%104);  
                                if (((fileelements[2]-20)%75)>40) fileelements[2]=fileelements[2]-((fileelements[2]-20)%75)+75;  
                                else fileelements[2]=fileelements[2]-((fileelements[2]-20)%75); 
                                fileelements[1]=fileelements[1]+b*104;
                            } else if (iconsize==3) { // 120 to 134 =Y
                                if (((fileelements[1]-20)%120)>80) fileelements[1]=fileelements[1]-((fileelements[1]-20)%120)+120;  
                                else fileelements[1]=fileelements[1]-((fileelements[1]-20)%120);  
                                if (((fileelements[2]-20)%95)>50) fileelements[2]=fileelements[2]-((fileelements[2]-20)%95)+95;  
                                else fileelements[2]=fileelements[2]-((fileelements[2]-20)%95);                         
                                fileelements[1]=fileelements[1]+b*120;
                            } else if (iconsize==4) {
                                if (((fileelements[1]-20)%140)>110) fileelements[1]=fileelements[1]-((fileelements[1]-20)%140)+140;  
                                else fileelements[1]=fileelements[1]-((fileelements[1]-20)%140);  
                                if (((fileelements[2]-20)%115)>60) fileelements[2]=fileelements[2]-((fileelements[2]-20)%115)+115;  
                                else fileelements[2]=fileelements[2]-((fileelements[2]-20)%115); 
                                fileelements[1]=fileelements[1]+b*140;                        
                            } else if (iconsize==5) {
                                if (((fileelements[1]-20)%166)>120) fileelements[1]=fileelements[1]-((fileelements[1]-20)%166)+166;  
                                else fileelements[1]=fileelements[1]-((fileelements[1]-20)%166);  
                                if (((fileelements[2]-20)%135)>70) fileelements[2]=fileelements[2]-((fileelements[2]-20)%135)+135;  
                                else fileelements[2]=fileelements[2]-((fileelements[2]-20)%135); 
                                fileelements[1]=fileelements[1]+b*166;                   
                            }

							if (fileelements[1]==0 && fileelements[2]==0) {  //fix empty positions
								fileelements[1]=Math.round(parseInt(icontops[tmp],10)); //top
                                fileelements[2]=Math.round(parseInt(iconlefts[tmp],10)); //left

							}

                            // arrange icons on desktop ***************************************************************************************
                            var tmpc=0;
                            for (tmpc=0;tmpc<tmp;tmpc++) {
                                if (icontops[tmpc]==fileelements[1] && iconlefts[tmpc]==fileelements[2]) {
                                    if (iconsize==1) {
                                        fileelements[1]=fileelements[1]+84;             
                                    } else if (iconsize==2) {
                                        fileelements[1]=fileelements[1]+104;
                                    } else if (iconsize==3) {
                                        fileelements[1]=fileelements[1]+120;
                                    } else if (iconsize==4) {
                                        fileelements[1]=fileelements[1]+140;               
                                    } else if (iconsize==5) {
                                        fileelements[1]=fileelements[1]+166;        
                                    }
									tmpc=0;
		    						if (fileelements[1]>500) {
			                            fileelements[1]=20;
									    if (iconsize==1) {
                                            fileelements[2]=fileelements[2]+55;             
                                        } else if (iconsize==2) {
                                            fileelements[2]=fileelements[2]+75;
                                        } else if (iconsize==3) {
                                            fileelements[2]=fileelements[2]+95;
                                        } else if (iconsize==4) {
                                            fileelements[2]=fileelements[2]+115;               
                                        } else if (iconsize==5) {
                                            fileelements[2]=fileelements[2]+135;        
                                        }
								    }
                                }
                            }
                            icontops[tmp]=Math.round(parseInt(fileelements[1],10)); //top
                            iconlefts[tmp]=Math.round(parseInt(fileelements[2],10)); //left
                            tmp++;
								
                            var FILENAME_title=fileelements[0].trim();

                            //set icon position on desktop
                           // $(".file-continer .file").eq(DestopFileEQ).css({"color":"white","top":fileelements[1]+"px","left":fileelements[2]+"px"});
							}  				
				} else {
					//desktop.ini not found

						///console.log("iconsize_2: "+iconsize);                
                        ...
                        $(".file-continer .file").each(function(nc,ncn) {
                            ...
                            iconlefts[tmp]=Math.round(parseInt(fileelements[2],10)); //left
                            tmp++;                       
					        $(".file-continer .file").eq(nc).css({"top":fileelements[1]+"px","left":fileelements[2]+"px"});
				        });				
						}
					...
					$(this).css({
                        top: Math.round(parseInt(fileelements[1],10)),
                        left: Math.round(parseInt(fileelements[2],10))

                        //zakazanie resetu pozicii ikoniek pri nacitavani refreshu
                        /*left: k,
                        top: l*/
                    });
...
//******************************************************************************  
				...
				if (isMobile==true) {
                    ...
                    $(".file-continer").css("overflow", "scroll");
                    $(".file-continer").css("display", "block");                   
                    $(".tabs").css("z-index", "100000"); 
                    $(".tabs").css("display", "block"); 
                    $(".bodymain").css("overflow", "scroll");
                    $(".bodymain").css("display", "table");
                }                 
                $("#menuwin #links").css("margin-left", "-5px");      

                $(".file-continer").css("padding", "0px"); //CSS FIX

//******************************************************************************  
//NCX: MAIN DESKTOP REFRESH            
//******************************************************************************                                         
				
				$(".systemBox").removeClass("systemBox").removeClass("menu-default").attr("data-path","Lw_c_c");
                //NCX: unlock system default icons       
...
//******************************************************************************                                         
//NCX: LOAD DESKTOP FROM FILE (everytime)
//******************************************************************************                                         
/*
				desktopinidata = load_datafile(iniurl);
				if (desktopinidata!="") {
                 /*   filelines = desktopinidata.split("\n");                                             
                    var fileelements="";
				    var DestopFileEQ=-1;
						...
						$(".file-continer .file").each(function(nc,ncn) {
                            ///console.log("nc "+nc+" : "+$(ncn).find(".filename .title").text().trim());
							...
							var filematch=false;
   							$.each( filelines, function( mc, mcm ) {
							    if (mcm!="") {
							        fileelements=mcm.split("|");
									if ($(".file-continer .file").eq(nc).find(".filename .title").text().trim()== fileelements[0].toString()) {
										//match file
										filematch=true;
										DestopFileEQ=nc;
										return(false);
									} else if ($(".file-continer .file").eq(nc).find(".filename .title").text().trim() == newfilename)  {
										//new File or Folder
										filematch=true;
										DestopFileEQ=nc;
								        fileelements[1]=parseInt(mouseY_new_icon,10);
...
/*						console.log("iconsize_2: "+iconsize);                
					...
					//}
                    //$(".file-continer").show(); //takto je to lepsie chvilku skryte ikonky boli zobrazene po upratani
//******************************************************************************                                                                 
//NCX:SAVE ALL TO FILE
//******************************************************************************                                                                 
                    setTimeout(function(){
                        desktopinidata="";
                        if ( desktopinidata=="") { 
                            $(".file-continer .file").each(function(nc,ncn) {                  
                                var icontop=$(this).css('top').replace('px', '');
                                var iconleft=$(this).css("left").replace('px', '');            
                                desktopinidata=desktopinidata+$(this).find(".filename .title").text().trim()+"|"+Math.round(icontop)+"|"+Math.round(iconleft)+"\n";
                            });
                        }
                        ...
                        save_datafile(iniurl,desktopinidata);
                        //desktop refresh request    
...
//NCX:folder ICON
//******************************************************************************                                         
                    $(".file-continer .folder-box").each(function(nc,ncn) {                  
                        var iconurl="";
                        if (G.myDesktop=="/desktop/") {
                            iconurl=("/"+"data/User/"+$("#topbar-user").text().trim()+"/home/desktop/"+$(this).find(".filename span").text().trim()+'/avatar.ico');
                        } else {
                            iconurl=G.myDesktop+$(this).find(".filename span").text().trim()+'/avatar.ico';
                            iconurl=core.path2url(iconurl);                  
                        }
						
						var self=$(this);
						
						if (checkurl(iconurl)==true) {
                        var success = load_datafile(iconurl);
      				    if (success) {	   
                            self.find(".ico").html("<i style='background:url(\""+iconurl+"\");width:100%;height:100%;display:table;background-size:contain'></i>");
                        } else {  } 
						}
                    });
//******************************************************************************     
    ...
    var c = a("./pathOperate"),
        ...
        p = function(b, d, e) { 
            ...
            "folder" != b && -1 != d.indexOf(".") && (n = d.length - e.length - 1), l.textSelect(0, n), "split" == G.userConfig.listType && l.css("width", l.parents(".filename").width() - 40), "icon" == G.userConfig.listType ? ($("#makefile").css({
                ...
                transition: "none"
            }), $("#makefile .textarea").css("margin-top", "-13px")) : $("#makefile .x-item-file").addClass("small"), core.isApp("desktop") /*&& ui.resetDesktopIcon()*/; //NCX: vyhodime refresh aby neliteali ikonky pri vytvarani
//*******************************************************************************************						
//NCX: New folder position:
		    mouseX_new_icon=getMouseX()-50;
    		mouseY_new_icon=getMouseY()-50;
            $(".file-icon-edit").css({"left":mouseX_new_icon+"px","top":mouseY_new_icon+"px"});
            newfilename=$(".file-continer .file-icon-edit").find(".filename .title").text().trim();
//*******************************************************************************************
            var p = function(a) {
...
//*******************************************************************************************
			...
			newfilenameY=$(b).css("top").replace('px', '');
			newfilenameX=$(b).css("left").replace('px', '');
//*******************************************************************************************
...
//******************************************************************************                                         
            $(Config.FileBoxClass).die("touchstart").live("touchstart", function(a, b, c, d) {
                var e = $(a.target);
                if (isMobile==false) e.hasClass("item-menu") || e.parent().hasClass("item-menu") || e.hasClass("item-select") || e.parent().hasClass("item-select") || e.parents(".children-more").exists() || ($(this).hasClass("select") ? ui.path.open() : (ui.fileLight.clear(), $(this).removeClass("select"), $(this).addClass("select"), ui.fileLight.select()))				
//******************************************************************************    
...
//******************************************************************************                                         
//NCX:FILEBOX DOUBLECLICK....
//******************************************************************************                                         
           }).die("dblclick").live("dblclick", function(a) {
               if (isMobile==true) { a.preventDefault(); ui.path.open(); }           
...
//******************************************************************************                                         
//NCX:MOBILEFIX:   MAIN for disable scrolling when dragging files
//******************************************************************************                                         
               $(Config.FileBoxClass).on('touchmove', function(e) {
                  e.preventDefault();				  
               }); 
//****************************************************************************** 				
                if (!b.shiftKey) {
...
//NCX:
//******************************************************************************                                                          
                   if (isMobile==false || tapMoved==true) {
                      $("body").addClass(p);
...
//******************************************************************************                                         
							...
							console.log("iconsize_3: "+iconsize); */
							iconsize = G.userConfig["fileIconSizeDesktop"];
							if (iconsize==40) iconsize=1; 
							else if (iconsize==60) iconsize=2; 
							else if (iconsize==80) iconsize=3; 
                            ...
                            ////var scrollTopY =  $(".file-continer").scrollTop();
                            var scrollTopY =  $(".file-continer").scrollTop();
                            var scrollLeftX =$(".file-continer").scrollLeft();                                                                                                     
                            var dragX =  $(".draggable-dragging").css("left").replace('px', '');    
                            var dragY =  $(".draggable-dragging").css("top").replace('px', '');
                        
                            var Yoffseticon=parseInt(dragY, 10) + parseInt(scrollTopY, 10);
...
//******************************************************************************                                                                 
                            if (isMobile==true) {                                
                               ...
                               $(".file-continer").css("height", "auto" );                            
                               $(".draggable-dragging").css("top", $(this).css("top").replace('px', '') + "px"   );
                               $(".draggable-dragging").css("left", $(this).css("left").replace('px', '') + "px"   );
                          //******************************************************************************                                                                 
                         } else { 
                            if (iconsize==1) {
                               $(this).css({
                                  left: c.left + b - 42 ,                                                          
                                  top: c.top + b + F - 42 ///+ scrollTopY -40                                     
                               });
                            } else if (iconsize==2) {
                               $(this).css({
                                  left: c.left + b - 52 ,                                                          
                                  top: c.top + b + F - 52 ///+ scrollTopY -40                                     
                               });
                            } else if (iconsize==3) {
                               $(this).css({
                                  left: c.left + b - 60 ,                                                          
                                  top: c.top + b + F - 60 ///+ scrollTopY -40                                     
                               });
                            } else if (iconsize==4) {
                               $(this).css({
                                  left: c.left + b - 70 ,                                                          
                                  top: c.top + b + F - 70 ///+ scrollTopY -40                                     
                               });
                            } else if (iconsize==5) {
                               $(this).css({
                                  left: c.left + b - 83 ,                                                          
                                  top: c.top + b + F - 83 ///+ scrollTopY -40                                     
                               });
                            }
...
//******************************************************************************                                                                 
                ...
                L = function(a) {
					...
					$(".file-draging .file").each(function(b) {
//******************************************************************************                                                                 
//NCX:DRAGGOFF + MOBILE FIX
//******************************************************************************                                                                 
                        var scrollTopY =  $(".file-continer").scrollTop();
                        var scrollLeftX =$(".file-continer").scrollLeft();                                                                                                
                        var dragX =  $(".draggable-dragging").css("left").replace('px', '');    
                        var dragY =  $(".draggable-dragging").css("top").replace('px', '');                      
                        var Xoffseticon=parseInt(dragX, 10) + parseInt(scrollLeftX, 10);
                        var Yoffseticon=parseInt(dragY, 10) + parseInt(scrollTopY, 10);

                        desktopinidata="";
                        /*if ($(".box-size-smallx").hasClass("selected")) var iconsize=1; 
                        else if ($(".box-size-small").hasClass("selected")) var iconsize=2; 
                        else if ($(".box-size-default").hasClass("selected")) var iconsize=3; 
                        else if ($(".box-size-big").hasClass("selected")) var iconsize=4; 
                        else if ($(".box-size-bigx").hasClass("selected")) var iconsize=5;*/
                        ...
                        var icontop=$(this).css('top').replace('px', '') - $('.topbar').css('height').replace('px', '');
                        var iconleft=$(this).css("left").replace('px', ''); 
                    
                        if (isMobile==true) {                                               
						    //icontop= Yoffseticon;        //fix file dragoff cords 
                            //iconleft=Xoffseticon;
                            if (iconsize==1) {
                                icontop= Yoffseticon-20;        //fix file dragoff cords 
                                iconleft=Xoffseticon;
                            } else if (iconsize==2) {
                                icontop= Yoffseticon-25;        //fix file dragoff cords 
                                iconleft=Xoffseticon;
                            } else if (iconsize==3) {
                                icontop= Yoffseticon-30;        //fix file dragoff cords 
                                iconleft=Xoffseticon;
                            } else if (iconsize==4) {
                                icontop= Yoffseticon-45;        //fix file dragoff cords 
                                iconleft=Xoffseticon;
                            } else if (iconsize==5) {
                                icontop= Yoffseticon-50;        //fix file dragoff cords 
                                iconleft=Xoffseticon;
                            }
                        } else {
                            if (iconsize==1) {
                                icontop= Yoffseticon-50;        //fix file dragoff cords 
                                iconleft=Xoffseticon-42;
                            } else if (iconsize==2) {
                                icontop= Yoffseticon-60;        //fix file dragoff cords 
                                iconleft=Xoffseticon-52;
                            } else if (iconsize==3) {
                                icontop= Yoffseticon-60;        //fix file dragoff cords 
                                iconleft=Xoffseticon-60;
                            } else if (iconsize==4) {
                                icontop= Yoffseticon-70;        //fix file dragoff cords 
                                iconleft=Xoffseticon-70;
                            } else if (iconsize==5) {
                                icontop= Yoffseticon-83;        //fix file dragoff cords 
                                iconleft=Xoffseticon-83;
                            }                    
                        }
                        //GRID MATICA---------------------------------------------------------------
                        if (iconsize==1) {
                            if (((icontop-20)%84)>40) icontop=icontop-((icontop-20)%84)+84;  
                            else icontop=icontop-((icontop-20)%84);  
                            if (((iconleft-20)%55)>30) iconleft=iconleft-((iconleft-20)%55)+55;  
                            else iconleft=iconleft-((iconleft-20)%55);    
                            icontop=icontop+b*84;                
                       } else if (iconsize==2) {
                            if (((icontop-20)%104)>70) icontop=icontop-((icontop-20)%104)+104;  
                            else icontop=icontop-((icontop-20)%104);  
                            if (((iconleft-20)%75)>40) iconleft=iconleft-((iconleft-20)%75)+75;  
                            else iconleft=iconleft-((iconleft-20)%75); 
                            icontop=icontop+b*104;
                       } else if (iconsize==3) { // 120 to 134 =Y
                            if (((icontop-20)%120)>80) icontop=icontop-((icontop-20)%120)+120;  
                            else icontop=icontop-((icontop-20)%120);  
                            if (((iconleft-20)%95)>50) iconleft=iconleft-((iconleft-20)%95)+95;  
                            else iconleft=iconleft-((iconleft-20)%95);                         
                            icontop=icontop+b*120;
                        } else if (iconsize==4) {
                            if (((icontop-20)%140)>110) icontop=icontop-((icontop-20)%140)+140;  
                            else icontop=icontop-((icontop-20)%140);  
                            if (((iconleft-20)%115)>60) iconleft=iconleft-((iconleft-20)%115)+115;  
                            else iconleft=iconleft-((iconleft-20)%115); 
                            icontop=icontop+b*140;                        
                        } else if (iconsize==5) {
                            if (((icontop-20)%166)>120) icontop=icontop-((icontop-20)%166)+166;  
                            else icontop=icontop-((icontop-20)%166);  
                            if (((iconleft-20)%135)>70) iconleft=iconleft-((iconleft-20)%135)+135;  
                            else iconleft=iconleft-((iconleft-20)%135); 
                            icontop=icontop+b*166;                   
                        }
                        t.parent().find(".select:eq("+b+")").css("top",icontop + "px");
                        t.css("left",iconleft + "px");
                        t.removeClass("item-file-draging");
					                        
					    $(this).css("left",$(this).data("data-left") + "px");
					    $(this).css("top",$(this).data("data-top") + "px");
					
					    var top_bar_offset = parseInt($('.topbar').css('height').replace('px', ''), 10);	
//NCX: ANIMATE MOVE!						
//*****************************************************************************					  
						var c = b * E,
                        ...
                        a ? $(this).stop().animate({
                            ...
                            left: iconleft - scrollLeftX, 
                            top: icontop+top_bar_offset-scrollTopY  //scrollTopY treba na scrolovaciu plochu
                        }, 250 + c, function() {
                            t.removeClass("item-file-draging")
                        }).animate({
                            opacity: 0
...
//******************************************************************************                                                                 
                        ...
                        save_datafile(iniurl,desktopinidata); //uloz move rozdiely
                        ui.resetDesktopIcon(); //uprac rozhadzane iconky
//******************************************************************************    
                    }, 400 + E * b);
            ...
            var m = function(e) {
                ...
                p = function() {
                    for (var a = c.offset().left - l.offset().left + l.scrollLeft(), b = c.offset().top - l.offset().top + l.scrollTop(), d = a + c.width(), e = b + c.height(), f = ui.fileLight.fileListAll(), g = 0; g < f.length; g++) {
                        var h = f[g],
                            i = $(f[g]),
                            j = 0, //i.parent().scrollTop(), //NCX: hnusoba jedna, blokovala selectovanie mimo obrazovku!
                            k = h.offsetLeft,
                            m = h.offsetTop - j,
                            n = k + i.width(),
                            o = m + i.height();
